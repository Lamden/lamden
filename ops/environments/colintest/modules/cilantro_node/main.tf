...
variable "region" {
  type        = "string"
  description = "The region to launch the node into"
}
...
variable "security_group" {
  type        = "string"
  description = "The name of the AWS security group to use for the instance"
}
...
locals {
  ...
  amis = {
    ...
    ap-northeast-3 = "ami-084e4de48dc0834e8"
    ...
    us-gov-east-1  = "ami-2cf5135d"
    us-gov-west-1  = "ami-af8de0ce"
  }
...
resource "aws_instance" "cilantro_ee_node" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo apt-get install socat",     # Instal socat (for issuing SSL certificates
...
resource "null_resource" "cilantro_ee-conf" {
  ...
  provisioner "file" {
    ...
    destination = "/etc/cilantro_ee.conf"
  }
}
...
resource "null_resource" "ssh-keys" {
  ...
  provisioner "file" {
    ...
    destination = "/home/ubuntu/authorized_keys.loc"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      "cat /home/ubuntu/authorized_keys.loc >> /home/ubuntu/authorized_keys",
      "sort authorized_keys | uniq > authorized_keys.loc",
      "mv authorized_keys{.loc,}",
    ]
  }
}
