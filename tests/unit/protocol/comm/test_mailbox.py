...
class TestAsyncServer(TestCase):
    ...
    def test_init(self):
        w = Wallet()
        services.AsyncInbox(services._socket('tcp://127.0.0.1:10000'), w, self.ctx)

    def test_addresses_correct(self):
        w = Wallet()
        m = services.AsyncInbox(services._socket('tcp://127.0.0.1:10000'), w, self.ctx)
    ...
    def test_sending_message_returns_it(self):
        w = Wallet()
        m = services.AsyncInbox(services._socket('tcp://127.0.0.1:10000'), w, self.ctx, linger=500, poll_timeout=500)
