...
class TestProcessingQueue(TestCase):
    ...
    def test_should_check_again(self):
        hlc_timestamp = "1"
        ...
        should_check = self.validation_queue.should_check_again(hlc_timestamp)

        self.assertTrue(should_check)

    def test_should_check_again_false(self):
        hlc_timestamp = "1"
        ...
        # Should check is false because we checked and no new solutions have come in
        should_check = self.validation_queue.should_check_again(hlc_timestamp)

        self.assertFalse(should_check)

    def test_should_check_again_after_updated_solution_from_peer(self):
        hlc_timestamp = "1"
        ...
        # resubmit the solution for the peer
        self.add_solution(
            verifying_key=peer_wallet.verifying_key,
            hlc_timestamp=hlc_timestamp,
            hash="2"
        ...
        # Should check is true because an updated solution came in from the peer
        should_check = self.validation_queue.should_check_again(hlc_timestamp)
    ...
    def test_process_next_ideal_consensus(self):
        hlc_timestamp = "1"
        self.num_of_peers = 1  # Does not include me
