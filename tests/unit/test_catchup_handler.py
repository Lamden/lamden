...
class TestCatchupHandler(TestCase):
    ...
    def test_PROPERTY_latest_block_number__returns_correct_block_height(self):
        ...
        block_number = 10
        self.contract_driver.driver.set(LATEST_BLOCK_HEIGHT_KEY, str(block_number))
    ...
    def test_METHOD_update_block_db__(self):
        self.create_catchup_handler()
        ...
        self.catchup_handler.update_block_db(block=block)

        latest_block_height = self.contract_driver.driver.get(LATEST_BLOCK_HEIGHT_KEY)
        latest_block_hash = self.contract_driver.driver.get(LATEST_BLOCK_HASH_KEY)

        self.assertEqual(block_number, latest_block_height)
        self.assertEqual(block_hash, latest_block_hash)
    ...
    def test_METHOD_get_highest_network_block__can_return_the_latest_block_of_all_peers(self):
        ...
        # Add 1 peer to the network with all blocks
        self.add_peers_to_network(amount=1, blocks=mock_blocks.blocks)

        # Remove 1 block and give that to 4 new peers
        mock_blocks.blocks.pop(mock_blocks.latest_block_number)
        self.add_peers_to_network(amount=4, blocks=mock_blocks.blocks)
        ...
        # This block should be the block that only the first peer added has.
        block = res[0]
    ...
    def test_METHOD_run__can_catchup_all_blocks_from_peers(self):
        ...
        # Each new node has 1 less block thank the last
        self.add_peers_to_network(amount=1, blocks=mock_blocks.blocks)
    ...
    def test_METHOD_run__does_not_run_if_at_latest(self):
        ...
        self.create_catchup_handler()
        self.catchup_handler.update_block_db(block=mock_blocks.latest_block)
