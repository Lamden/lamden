...
from contracting.db.driver import FSDriver, ContractDriver, TIME_KEY
...
class TestCreateGenesisBlock(TestCase):
    ...
    @patch('lamden.utils.create_genesis.confirm_and_flush_blocks', return_value=True)
    def test_migration_scheme_filesystem_preserves_existing_state(self, flush_blocks):
    ...
    @patch('lamden.utils.create_genesis.confirm_and_flush_blocks', return_value=True)
    def test_migration_scheme_filesystem_resubmits_genesis_contracts(self, flush_blocks):
        setup_genesis_contracts(self.contracting_client)
        earlier_time = Datetime(year=1, month=1, day=1)
        for contract in GENESIS_CONTRACTS:
            self.state.set(f'{contract}.{TIME_KEY}', earlier_time)
        ...
        for contract in GENESIS_CONTRACTS:
            self.assertGreater(self.state.get(f'{contract}.{TIME_KEY}'), earlier_time)
