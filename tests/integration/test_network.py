    ...
    def test_connect(self):
        try:
            ...
            self.await_async_process(network1.connect,
                                     {'ip':'tcp://127.0.0.1', 'key': wallet2.verifying_key, 'ports': network2.socket_ports})
            self.async_sleep(1)
            ...
            print(datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3] + ': ' + 'sending test message')
            network1.peers[wallet2.verifying_key].dealer.send_msg('test')
            # network2.router.send_msg(str('"' + wallet1.verifying_key + '"').encode(), '{"response":"test"}'.encode())

            self.async_sleep(1)
            self.await_async_process(network2.publisher.publish, {'topic':'', 'msg': 'test publish'})
            self.async_sleep(1)

            print(datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3] + ': ' + 'stopping')
            network1.stop()
            network2.stop()
            self.async_sleep(1)
            self.assertFalse(network1.running)
